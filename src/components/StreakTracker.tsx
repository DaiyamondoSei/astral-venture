...
const StreakTracker = ({
  ...
  const weekDaysArranged = Array.from({ length: 7 }, (_, i) => {
    const dayIndex = i;
    ...
    <Card className="glass-card shadow-lg">
      ...
      <CardContent>
        <div className="grid grid-cols-7 gap-2 mb-4">
          {weekDaysArranged.map((day, index) => (
            <div key={index} className="flex flex-col items-center">
              ...
              <div 
                className={cn(
                  ...
                  day.isActive 
                    ? "bg-gradient-to-br from-quantum-400/80 to-quantum-700" 
                    : day.isMissed
                      ? "bg-black/30 border border-red-500/30"
              ...
              <div className="text-xs mt-1 font-medium" style={{color: `${day.chakraColor}CC`}}>
                {day.chakraName}
              </div>
            </div>
          ))}
        ...
        {weekDaysArranged[currentDayIndex] && !weekDaysArranged[currentDayIndex].isActive ? (
          <Button 
            className="w-full bg-gradient-to-r from-quantum-400 to-quantum-700"
            onClick={() => onChakraActivation && onChakraActivation(currentDayIndex)}
          >
            <Zap size={16} className="mr-2" />
            Activate Today's Chakra
          </Button>
        ) : weekDaysArranged.some(day => day.isMissed) ? (
          <Button 
            variant="outline" 
            className="w-full border-quantum-400/30 text-quantum-400"
            onClick={() => onRecalibration && onRecalibration()}
          >
